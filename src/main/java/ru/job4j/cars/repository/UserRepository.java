...
public class UserRepository {
    ...
    public List<User> findByLikeLogin(String key) {
        Session session = sf.openSession();
        List<User> list = session.createQuery("from User as u where u.login like '%" + key + "%'").list();
        return list;
    }
    ...
    public Optional<User> findByLogin(String login) {
        Session session = sf.openSession();
        Optional<User> user = session.createQuery("from User as u where u.login='" + login + "'").stream().toList().stream().findFirst();
        return user;
    }
}
